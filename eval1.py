...
def parse_args(argv=None):
    ...
    parser.add_argument('--dimension', default=None, type=str , help='To reduce the processing time.')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if undo_transform:
        ...
        #print("image_numpy " + str(img_numpy))
        #print("image_gpu " + str(img_gpu))
    else:
        img_gpu = img / 255.0
        h, w, _ = img.shape
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        #print("number of masks_colors " + str(len(masks_color)))

        #print("length of masks_color - x axis " + str(len(masks_color[0][0])))
        #print("length of masks_color - y axis " + str(len(masks_color[0])))



        #for i in range(0,len(masks_color)):
            #print(masks_color[i][140][150])
            
        

        #print("example " +str(masks_color[0][0][1]))

        # This is 1 everywhere except for 1-mask_alpha where the mask is
        inv_alph_masks = masks * (-mask_alpha) + 1
        #print(len(inv_alph_masks[0]))
        #print(len(inv_alph_masks[0][0]))
        #print(inv_alph_masks[0][200][500])
        ...
        #print("masks_color_summand is " + str(masks_color_summand))
        #print(masks_color_summand[40][150])
        #print(masks_color_summand[100][100])
        #print(len(masks_color_summand))
        #print(len(masks_color_summand[0]))


        #nump = []
        #for i in range(0,len(masks_color_summand)):
            #nump.append([])


        #for y in range(0,len(masks_color_summand)):
            #for x in range(0,len(masks_color_summand[0])):
                #if(masks_color_summand[y][x][0]!=0):
                    #nump[y].append(1)
                #else:
                    #nump[y].append(0)
        #print(len(nump))
    ...
    def best_fit_slope_and_intercept(xs,ys):
            ...
            b = mean(ys) - m*mean(xs)
            return m, b
    #print("number dets to consider : " +  str(num_dets_to_consider))
    ...
    if args.display_text or args.display_bboxes:
        ...
        for j in reversed(range(num_dets_to_consider)):
            ...
            width_box = abs(x2 - x1)
            height_box = abs(y1 - y2)
            b_boxes.append([mid_x, mid_y, width_box, height_box])
            ...
            #print(color)
            if args.display_bboxes and _class == 'car':
                #print(len(img_numpy))
                ...
                x_min = x1
                x_max = x2
                vehicle_area = abs(y_difference * (x_max-x_min))
                #print("vehicle area is : " + str(vehicle_area))
                #print("invers 100 is " + str(inv_alph_masks[0][100][100]))
                #print("masks 100 is " + str(masks[0][100][100]))
                #print(x_min, x_max)	
                #print(y_difference)
                ...
                if(0.85>percentage>0 and score>0.6 and _class == 'car'):
                    ...
                    left_x_values = []
                    for i in range(0,len(left_y_values)):
                    ...
                    #RuntimeError: CUDA out of memory. Tried to allocate 60.00 MiB (GPU 0; 2.00 GiB total capacity; 1.11 GiB already allocated; 17.62 MiB free; 1.22 GiB reserved in total by PyTorch)
                    #print(masks[0][100][170])
        ...
        #proto_tensor = tf.make_tensor_proto(temp_numpy)
        #temp_array = tf.make_ndarray(proto_tensor)
	
        #f_features = np.array(encoder(original_image, b_boxes))
		
        #detections = [Detection(bbox, score, class_name) for bbox, score, class_name in zip(b_boxes, s_scores, n_names)]

        # Pass detections to the deepsort object and obtain the track information.
        #tracker.predict()
        #tracker.update(detections)
	#instant = instant +1
    #imsave('out/frame%instant.jpg' % instant,img_numpy)           
    #im = Image.fromarray(img_numpy)
    #im.save('out/frame.jpg')

    #del count
    #del masks
    #del masks_color_summand
    #del inv_alph_masks
    #del num_dets_to_consider
    #del m_y_max
    #del m_max
    #del m_min
...
def evalimage(net:Yolact, path:str, save_path:str=None, dim= None):
    #print(type(dim[0]))
    ...
    relevant_numpy = cv2.imread("temp/temp.jpg")
    #cv2.imshow("temp1.jpg",relevant_numpy)
    temp_numpy[:,:]= (255,255,255)
    #rows,cols,channels = relevant_numpy.shape

    #roi = temp_numpy[int(dim[2]):int(dim[3]),int(dim[0]):int(dim[1])]
    #logoGray = cv2.cvtColor(relevant_numpy,cv2.COLOR_BGR2GRAY)
    ...
    #frontImage = cv2.bitwise_and(relevant_numpy, relevant_numpy, mask = inverseMask)

    #dst = cv2.add(background, frontImage)
    
    #temp_numpy[int(dim[2]):int(dim[3]),int(dim[0]):int(dim[1])] = dst
    temp_numpy[int(dim[2]):int(dim[3]),int(dim[0]):int(dim[1])] = relevant_numpy
    #cv2.imshow("temp.jpg",temp_numpy)
    
    original_image = cv2.cvtColor(temp_numpy, cv2.COLOR_BGR2RGB)
    ...
    img_numpy = prep_display(preds, frame1, None, None, undo_transform=False)
...
def evaluate(net:Yolact, dataset, dim = None, train_mode=False):
    ...
    if args.image is not None:#For single image
        print("1st_1st")
        if ':' in args.image:#this condition works when we wants to save the processed image
            print("2nd_1st")
            inp, out = args.image.split(':')
            print("inp " + str(inp) )
            print("out " + str(out))
            evalimage(net, inp, out, dim=dim)
        else:#this condition works when we wants only to display the processed image
            print("2nd_2nd")
            
            evalimage(net, args.image, dim=dim)
        return
    elif args.images is not None:#for group of images
        print("1st_2nd")
        ...
        print("1st_3rd")
        if ':' in args.video:#this condition works when we wants to save the processed video
